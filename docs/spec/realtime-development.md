...
- 只有用户明确要求自动化覆盖时，才在 `apps/live/tests` 增加对应单元或集成测试。
- 开发 Agent 可以运行当前工具链已有的类型检查或 Vitest 来定位问题，但这些检查不是部署前置条件。
- 不固定运行 coverage、lint、类型、测试和 build 的完整组合，也不让 Tester 重跑这些检查。
- 实时运行时改动完成后立即部署；独立 Tester 子 Agent 在测试环境验证实际连接、传播、权限、持久化结果和直接风险。
...
- 相关实时场景已由未参与实现的 Tester 子 Agent 独立验收；不依赖运行环境的改动无需部署。
