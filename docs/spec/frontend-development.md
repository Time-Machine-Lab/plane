...
开发 Agent 可以运行与改动直接相关的快速检查来定位问题；受影响应用的 lint、类型、构建和已有自动化测试由
CI 统一提供合并证据。Storybook 只在共享组件隔离行为需要验证时运行。不要把 format、lint、types、build 和
Storybook 固定组合成每次变更的完整命令矩阵，也不要为单个页面临时引入新的测试框架。

独立 Tester 子 Agent 根据需求选择静态、离线、本地或测试环境验证。只有目标必须依赖真实路由、权限、异步
状态或浏览器交互时才部署，并只检查受影响视口和必要相邻行为，不重复实现 Agent 的开发检查。
...
- 需求涉及的场景和视口已由未参与实现的 Tester 子 Agent 独立验收；无需运行环境的改动不要求部署。
