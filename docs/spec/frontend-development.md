...
开发 Agent 可以在实现过程中运行当前工具链已经具备的快速检查来定位问题，但这不是完成阶段或部署前置条件。
受影响应用的 lint、类型、构建和已有自动化测试由 CI 提供合并证据；只有用户明确要求时才在
本次任务中执行。不要把 format、lint、types、build 和 Storybook 固定组合成命令矩阵。

运行时前端实现完成后立即部署受影响服务。独立 Tester 子 Agent 在测试环境验证真实路由、权限、异步状态、
浏览器交互、受影响视口和必要相邻行为，不重复 lint、类型、构建或自动化测试。
