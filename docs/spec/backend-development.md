...
- Ruff 描述代码风格，但本地主机运行 Ruff 不是默认完成条件；只有用户明确要求时才执行 Ruff 或后端自动化测试。
...
- 权限、数据隔离、迁移和复杂状态通过测试环境中的独立场景验收覆盖；后端自动化测试只有用户明确要求时才新增或运行。
- 运行时改动完成后已部署受影响服务，并由未参与实现的 Tester 子 Agent 在测试环境完成独立验收。
